            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 8) {
                    ForEach(Array(mockAlerts.enumerated()), id: \.offset) { index, alert in
                        AlertRow(emoji: alert.emoji, title: alert.title, body: alert.body, time: alert.time, isNew: index < 2)
                    }
                }
                .padding(.horizontal, 20)
                .padding(.bottom, 90)
...
struct SettingsHeader: View {
    ...
    var body: some View {
        HStack {
            Text(title).font(.system(size: 10.5, weight: .semibold)).foregroundColor(.white.opacity(0.3)).kerning(1)
            Spacer()
        }
